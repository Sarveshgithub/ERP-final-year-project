...
include 'connection.php';
 //$tablename =$_SESSION["un"];
  ...
  if(isset($_GET["f_id"]))
  {
$s="delete from fac_info where f_id='".$_GET["f_id"]."'";
$result=$conn->query($s);	 
if($result)
{
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h1 class="page-header">Faculty Record</h1>
...
<option>ME</option>
		   ...
		   if(isset($_POST["record"]))
	{
			...
			$q="select * from fac_info where dept='$branch';";
			$result = $conn->query($q);
           ...
           <table class="table " style="text-align:left;" border="2">
            <tr style="color:#000; background-color:#FFF;"><td><h3>Faculty Id</h3></td><td>
            <h3>Name</h3></td><td><h3>Department</h3></td><td><h3>Mail</h3></td><td><h3>Phone</h3></td>
            <td><h3>Address</h3></td><td><h3>Edit</h3></td><td><h3>Delete</h3></td></tr>
          <?php
  ...
  {?>
	  ...
	  <tr>
	 	   <?php echo '<td>'.$row["f_id"].'</td>';
				...
				echo '<td>'.$row["add"].'</td>';
				echo '<td>
	<a  href="admin_facinfoedit.php?rno='.$row["f_id"].'&name='.$row["name"].
	'&dept='.$row["dept"].'&mail='.$row["mail"].'&mno='.$row["mno"].
	'&add='.$row["add"].'">Edit</a></td>';
				echo '<td><a href="admin_facinfo.php?f_id='.$row["f_id"].'">Delete</a></td>';
